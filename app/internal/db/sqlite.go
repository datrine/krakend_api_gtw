...
func SetupRemoteDbLocalReplicatorsConnection(dbName, primaryUrl, authToken string) (*sql.DB, error) {
	...
	Migrate(db, "./internal/db/migrations/sqlite")
	return db, nil
}
...
func SetupRemoteDbConnection() (*sql.DB, error) {
	url := strings.Join([]string{config.GetTursoDatabaseURL() + "?authToken",
		config.GetTursoAuthToken()}, "=")
	fmt.Println(url)
...
func NewRemoteSqlLiteSQLDB() *SqlLiteSQLDB {
	db, err := SetupRemoteDbConnection()
	if err != nil {
		panic(err.Error())
	}
	return &SqlLiteSQLDB{
...
func NewRemoteWithLocalPrimarySqlLiteSQLDB() *SqlLiteSQLDB {
	primaryUrl := config.GetTursoDatabaseURL()
	authToken := config.GetTursoAuthToken()
	db, err := SetupRemoteDbLocalReplicatorsConnection("local.db", primaryUrl, authToken)
	if err != nil {
		panic(err.Error())
	}
	return &SqlLiteSQLDB{
...
func (d *SqlLiteSQLDB) CreateUser(ctx context.Context, userTocreate *sharedexports.CreateUser) error {
	fmt.Println(userTocreate.Email)
	result, err := d.DB.ExecContext(ctx, `INSERT INTO users 
	    ( email, password_hash, last_name, first_name) 
	VALUES ( ?, ?, ?, ?);`,
		//sql.Named("email", userTocreate.Email),
		userTocreate.Email,
		//sql.Named("password_hash", userTocreate.PasswordHash),
		userTocreate.PasswordHash,
		//sql.Named("last_name", userTocreate.LastName),
		userTocreate.LastName,
		//sql.Named("first_name", userTocreate.FirstName),
...
func (d *SqlLiteSQLDB) QueryUserByEmail(ctx context.Context, email string) (*sharedexports.User, error) {
	...
	row := d.DB.QueryRowContext(ctx, "SELECT * FROM users WHERE email= ?", sql.Named("email", email))
	err := row.Scan(rd...)
	if err != nil {
	...
	user.PasswordHash = passwordHashR
...
func (d *SqlLiteSQLDB) UpdateUser(ctx context.Context, email string, updates *sharedexports.UpdateUser) error {
	...
	if updates.FirstName != "" {
		if updateSql == "" {
			...
			updateSql = strings.Join([]string{updateSql, "first_name = ?"}, ", ")
		}
	...
	if updates.LastName != "" {
		if updateSql == "" {
			...
			updateSql = strings.Join([]string{updateSql, "last_name = ?"}, ", ")
		}
	...
	result, err := d.DB.ExecContext(context.Background(), "UPDATE users SET "+updateSql+" WHERE email = ?;",
		sqlNamedList...)
	if err != nil {
		return err
	}
